...
-(void)apiCallToPost:(NSString *)url serializedData:(NSData *)data callback:(void (^)(NSDictionary *))handler {
    ...
    if ([CheckInternetConnection connectedToNetwork]) {
        ...
        NSString *validUrl = [url stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]];
        
        if (validUrl) {
            [self request:validUrl serializedData:data callback:handler];
        }
...
-(void)request:(NSString *)url serializedData:(NSData *)data callback: (void(^)(NSDictionary *))handler {
