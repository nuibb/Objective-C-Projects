...
-(void)apiCallToGet:(NSString *)url callback: (void(^)(NSDictionary *))handler{
    if ([CheckInternetConnection connectedToNetwork]) {
        NSLog(@"url 123 : %@", url);
        if ([url stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]]) {
            [self request:[url stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]] callback:handler];
        }
    } else {
    ...
    NSURLSessionTask *task = [session dataTaskWithURL: [NSURL URLWithString:url]
                                    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                                        ...
                                        if (error != nil) {
                                            ...
                                            //NSLog(@"aaa :  %@",[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
...
-(void) processResponseUsingData:(NSData*)data callback: (void(^)(NSDictionary *))handler{
    ...
    if (!parseJsonError){
        ...
        // NSLog(@"%@", parseJsonError.localizedDescription);
